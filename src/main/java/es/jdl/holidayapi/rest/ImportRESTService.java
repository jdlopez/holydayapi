...
import es.jdl.holidayapi.services.importers.*;
import es.jdl.holidayapi.persistence.ConfigDao;
...
public class ImportRESTService {
    ...
    public Map<String, String> importHolydaysCSV(@PathVariable String prefix,
        ...
        if (!ret.isEmpty()) {
            ...
            List<Holiday> holidays = importerCSV.readFromSource();
            if (holidays == null || holidays.isEmpty())
                throw new RuntimeException("Holydays not found at: " + csvUrl);
            Counters counts = saveAllHolydays(holidays, ret.get("country"), ret.get("region"), ret.get("city"), debug);
            ret.put("holydays_read", String.valueOf(holidays.size()));
    ...
    private Counters saveAllHolydays(List<Holiday> holidays, String country, String region, String city, boolean debug) {
    ...
    public Map<String, String> importHolydaysScraper(@PathVariable String prefix,
        ...
        if (!ret.isEmpty()) {
            ...
            List<Holiday> holidays = importer.readFromSource();
            if (holidays == null || holidays.isEmpty())
                throw new RuntimeException("Holydays not found at: " + conf.getUrl());
            Counters counts = saveAllHolydays(holidays, conf.getCountry(), conf.getRegion(), conf.getCity(), debug);
            ret.put("holydays_read", String.valueOf(holidays.size()));
            ret.put("holydays_CountLocal", String.valueOf(counts.holydayCountLocal));
            ret.put("holydays_CountRegional", String.valueOf(counts.holydayCountRegional));
            ret.put("holydays_CountCountry", String.valueOf(counts.holydayCountCountry));
    ...
    private int insertHolyday(Holiday h) {
